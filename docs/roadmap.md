...
- [x] Analizar secuencialmente con MAEST la selección visible de Biblioteca, con
  ...
  resultados exclusivamente en memoria sin aplicarlos ni persistirlos.
- [x] Revisar y aplicar explícitamente las propuestas MAEST del lote visible. El
  usuario selecciona género y subgénero de forma independiente por pista, compara
  valor actual y propuesto y confirma antes de persistir. El servidor limita la
  operación a 25 pistas, valida evidencia MAEST v3, aísla por `user_id` y usa
  compare-and-set por campo para no sobrescribir cambios posteriores; conflictos
  y fallos parciales no revierten los campos ya aplicados. Esta acción solo
  actualiza metadatos persistidos y su procedencia automática; no escribe audio.
- [ ] Completar la clasificación local de escritorio con MAEST: escritura de
  otras etiquetas portables y ampliación del lote más allá de la selección
  visible cuando el rendimiento y la experiencia estén validados. Conservar
  audio y rutas exclusivamente en el dispositivo y comparar CPU y aceleración
  disponible antes de habilitar análisis masivo de toda la biblioteca.
