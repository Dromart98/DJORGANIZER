  ...
  .superRefine((condition, context) => {
    ...
    if (condition.field === "tag") {
      if (
        ...
        !z.string().uuid().safeParse(condition.value).success
      ) {
    ...
    if (numericField.has(condition.field)) {
      ...
      if (
        ...
        (typeof condition.value2 !== "number" || !Number.isFinite(condition.value2))
      ) {
      ...
      if (range) {
        for (const [path, value] of [
          ...
          if (typeof value === "number" && (value < range[0] || value > range[1])) {
            context.addIssue({
              code: "custom",
              message: `Usa un valor entre ${range[0]} y ${range[1]}.`,
              path: [path],
            });
          }
        }
...
export const smartCrateGroupSchema = z.object({
  logic: z.enum(["and", "or"]),
  conditions: z
    .array(smartCrateConditionSchema)
    .min(1)
    .max(SMART_CRATE_MAX_CONDITIONS),
});
...
export const smartCrateRulesSchema = z
  .object({
    ...
    logic: z.enum(["and", "or"]),
    groups: z
      .array(smartCrateGroupSchema)
      .min(1)
      .max(SMART_CRATE_MAX_GROUPS),
  })
  .superRefine((rules, context) => {
    const total = rules.groups.reduce(
    ...
    if (total > SMART_CRATE_MAX_CONDITIONS) {
      context.addIssue({
        code: "custom",
        message: `Usa como máximo ${SMART_CRATE_MAX_CONDITIONS} condiciones.`,
        path: ["groups"],
      });
    }
  });
...
export function parseSmartCrateRulesJson(value: string) {
  try {
    ...
    return smartCrateRulesSchema.safeParse(null);
  }
}

export type SmartCrateResolvedIds = {
  count: number;
  trackIds: string[];
};

export type SmartCrateResolvedPage = SmartCrateResolvedIds & {
  tracks: Tables<"tracks">[];
};

export async function resolveSmartCrateTrackIds(
  supabase: SupabaseClient<Database>,
  rules: SmartCrateRules,
  options: { limit?: number; offset?: number; search?: string } = {},
): Promise<SmartCrateResolvedIds> {
...
export async function resolveSmartCrateTracks(
  ...
  const resolved = await resolveSmartCrateTrackIds(supabase, rules, options);
  if (!resolved.trackIds.length) {
  ...
  const { data: tracks, error: trackError } = await supabase
    .from("tracks")
    .select("*")
    .in("id", resolved.trackIds);
  if (trackError) {
    throw new Error("No se pudieron cargar las pistas del crate inteligente.");
  }
  const byId = new Map((tracks ?? []).map((track) => [track.id, track]));
  return {
    ...resolved,
    tracks: resolved.trackIds.flatMap((id) => byId.get(id) ?? []),
  };
}

export async function resolveAllSmartCrateTrackIds(
  supabase: SupabaseClient<Database>,
  rules: SmartCrateRules,
) {
  ...
  for (let offset = 0; ; offset += 500) {
    const page = await resolveSmartCrateTrackIds(supabase, rules, {
      limit: 500,
      offset,
    });
    total = page.count;
    trackIds.push(...page.trackIds);
    if (trackIds.length >= total || !page.trackIds.length) break;
  }
  return trackIds;
}
