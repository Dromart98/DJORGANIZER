...
import { exportSmartCrateAction } from "@/app/crates/actions";
...
function summary(locale: Locale, track: Tables<"tracks">) {
  const duration = track.duration_seconds === null
    ? translate(locale, "Duración —")
    : formatDuration(Math.round(track.duration_seconds));
  return `${track.bpm ? `${track.bpm} BPM` : "BPM —"} · ${track.musical_key ?? `${translate(locale, "Tonalidad")} —`} · ${duration}`;
}
...
export async function SmartCrateDetail({
  ...
  const text = locale === "en" ? {
    eyebrow: "Smart crate",
    description: "This crate updates automatically from its saved rules. No tracks are copied into it.",
    matching: "matching tracks",
    search: "Search these results",
    searchAction: "Search",
    reset: "Clear",
    emptyTitle: "No tracks match these rules",
    emptyDescription: "Edit the rules or update your library metadata. Results refresh automatically.",
    edit: "Rules",
    ...
    description: "Este crate se actualiza automáticamente a partir de sus reglas guardadas. No copia pistas dentro del crate.",
    matching: "pistas coincidentes",
    search: "Buscar en estos resultados",
    searchAction: "Buscar",
    reset: "Limpiar",
    emptyTitle: "Ninguna pista cumple estas reglas",
    emptyDescription: "Edita las reglas o actualiza los metadatos de tu biblioteca. Los resultados se refrescan automáticamente.",
    edit: "Reglas",
    export: "Exportar resultados actuales",
    previous: "Anterior",
    next: "Siguiente",
  };
  ...
  return (
    <>
      <PageHeader
        action={
          <DeleteCrateForm crateId={crate.id} name={crate.name} revision={crate.updated_at} />
        }
      ...
      <section className="organization-layout">
        <div>
          ...
          <form className="library-toolbar" method="get">
            ...
            <button className="button button--secondary" type="submit">{text.searchAction}</button>
            {search ? <Link className="button button--ghost" href={`/crates/${crate.id}`}>{text.reset}</Link> : null}
          </form>
            ...
            <div className="crate-track-list">
              {resolved.tracks.map((track) => (
                <article className="card crate-track" key={track.id}>
                  ...
                  <Link className="button button--ghost" href={`/library/${track.id}`}>
                    {locale === "en" ? "View" : "Ver"}
                  </Link>
                </article>
              ))}
          ...
          {pageCount > 1 ? (
            <nav className="desktop-scan-pagination" aria-label={locale === "en" ? "Smart crate pages" : "Páginas del crate inteligente"}>
              {requestedPage > 1 ? (
                <Link className="button button--secondary" href={`/crates/${crate.id}?${new URLSearchParams({ ...(search ? { q: search } : {}), cratePage: String(requestedPage - 1) }).toString()}`}>{text.previous}</Link>
              ) : <span />}
              <span>{requestedPage.toLocaleString(locale)} / {pageCount.toLocaleString(locale)}</span>
              {requestedPage < pageCount ? (
                <Link className="button button--secondary" href={`/crates/${crate.id}?${new URLSearchParams({ ...(search ? { q: search } : {}), cratePage: String(requestedPage + 1) }).toString()}`}>{text.next}</Link>
              ) : <span />}
            </nav>
          ) : null}
          ...
          <div className="card organization-form">
            ...
            <div className="organization-inline-actions">
              {(["csv", "m3u", "m3u8"] as const).map((format) => (
                <form action={exportSmartCrateAction} key={format}>
                  <input name="crateId" type="hidden" value={crate.id} />
                  <input name="format" type="hidden" value={format} />
                  <button className="button button--secondary" type="submit">{format.toUpperCase()}</button>
                </form>
              ))}
            </div>
          </div>
