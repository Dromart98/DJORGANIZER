...
import {
  createPostAnalysisCrateAction,
  type CreatePostAnalysisCrateResult,
...
export function MaestBatchAnalysis({ tracks }: { tracks: MaestBatchTrack[] }) {
  ...
  const [crateResult, setCrateResult] =
    useState<CreatePostAnalysisCrateResult | null>(null);
  ...
  const counts = state?.items.reduce(
    ...
    { completed: 0, skipped: 0, failed: 0, cancelled: 0, pending: 0 },
  );
    ...
    state?.items.flatMap((item) => {
      ...
      const genre =
        selection?.genre && proposal.genre
          ? {
              expectedValue: item.evidence.genre.value,
              evidence: proposal.genre,
            }
          : undefined;
      const subgenre =
  ...
  const selectedFieldCount = applyRequestItems.reduce(
    (total, item) =>
      total + (item.genre ? 1 : 0) + (item.subgenre ? 1 : 0),
    0,
  );
            ...
            <>
              <div aria-live="polite">
                ...
                {counts ? (
                  <p>
                    {locale === "en"
                      ? `Completed: ${counts.completed} · Skipped: ${counts.skipped} · Failed: ${counts.failed} · Cancelled: ${counts.cancelled} · Pending: ${counts.pending}`
                      : `Completadas: ${counts.completed} · Omitidas: ${counts.skipped} · Fallidas: ${counts.failed} · Canceladas: ${counts.cancelled} · Pendientes: ${counts.pending}`}
                  </p>
                ) : null}
              </div>
              {state.error ? (
                      ...
                      {proposal?.genre ? (
                        <label className="maest-batch__option">
                          <input
                            ...
                            type="checkbox"
                          />
                          {locale === "en" ? "Apply genre" : "Aplicar género"}: {" "}
                          {item.evidence.genre.value ?? "—"} → {proposal.genre.value}
                        </label>
                      ) : null}
                      {proposal?.subgenre ? (
                        <label className="maest-batch__option">
                          ...
                          {locale === "en"
                            ? "Apply subgenre"
                            : "Aplicar subgénero"}
                          : {item.evidence.subgenre.value ?? "—"} → {" "}
                          {proposal.subgenre.value}
                        </label>
                      ) : null}
                      {applied ? (
                        <span role="status">
